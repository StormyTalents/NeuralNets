...
def model(learning_rate=0.01):
    # Parameters
    weights = {
        'conv1': tf.Variable(tf.random_normal([5, 5, 1, 8], stddev=np.sqrt(2./(5*5*1)))),
        'conv2': tf.Variable(tf.random_normal([5, 5, 8, 16], stddev=np.sqrt(2./(5*5*8)))),
        'out': tf.Variable(tf.random_normal([7*7*16, 10], stddev=np.sqrt(2./(7*7*16)))),
    }
    biases = {
    ...
    # Output FC
    out = tf.reshape(pool2, [-1, weights['out'].get_shape().as_list()[0]])
...
with tf.Session(config=config) as session:
    ...
    while not done:
        ...
        if epoch >= max_epochs or sustained_loss < loss_threshold:
            ...
            update_output(iteration, weights_val, loss_values, accuracy_values, override=True)
